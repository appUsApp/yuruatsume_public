...
*ステータス: App Store 審査通過済(リリース準備中)*

### 公開用設定

* 公開リポジトリでは AdMob 等で **ダミー ID** を使用しています。
* 多忙により、appleアカウントログインによるデータ引き継ぎ機能と端末間サイズによるUIの調整等がこちらのPiblicでは反映されていません。
...
* [0. TL;DR（30秒で概要）](#0-tldr30秒で概要)
* [1. アプリリンク / スクリーンショット](#1-アプリリンク--スクリーンショット)
* [2. プロジェクト概要](#2-プロジェクト概要)
* [3. 主なゲーム要素](#3-主なゲーム要素)
* [4. 技術スタック](#4-技術スタック)
* [5. アーキテクチャ / 設計ハイライト](#5-アーキテクチャ--設計ハイライト)
* [6. 私の担当（個人開発）](#6-私の担当個人開発)
* [7. チャレンジ＆解決](#7-チャレンジ解決)
* [8. 今後のロードマップ](#8-今後のロードマップ)
* [9. データ / プライバシー](#9-データ--プライバシー)
* [10. ストア状況](#10-ストア状況)
* [11. 作者 / 連絡先](#11-作者--連絡先)
* [付録: スクリーンショット一式](#付録-スクリーンショット一式)
* [付録:「企画〜UI/UX〜リリース準備まで広く取り組んだto-doリスト」](#付録-スクリーンショット一式)
...
『ゆるあつめ』は、ビーチを指でなぞって見つけて・集めて、あなただけのギャラリーを完成させる癒しのコレクションゲームです。朝・昼・夕・夜と移ろう時間帯で背景と BGM がシームレスに遷移し、出現物やイベントが変化します。シンプル操作と短いプレイサイクルを重視し、数分で小さな達成感を積み重ねられる設計にしています。
...
プレイアブルマップ **55+ 枚**, アイテム **200+**, シオノコ **260 体**（概数）。
...
収集物がページに蓄積され、**進捗に応じてビジュアルが変化**（69 枚 × 5 段階）。
...
集めたアイテムやシオノコ、背景・絵画・エフェクト・BGMを選択し、**自分だけのギャラリー**を作成。
...
申請/承認、ギャラリー訪問、**いいね（Friend Points）**。フレンドが設定した出現物で探索可能。
...
* **BaaS**: Firebase（Auth / Firestore / Functions v2）
* **広告/IAP**: Google Mobile Ads（Rewarded）/ StoreKit
* **CI/その他**: Xcode / SPM / 画像リソース管理（Assets）
...
* Firestore セキュリティ: `isSignedIn` / `isOwner` 等で **所有権チェック**
* ストア審査向けに **デバッグフラグ無効化ビルド** を用意
* 時間帯 / BGM / 背景 / 出現テーブルを **一元管理** する設計
...
* **アプリ実装**（SwiftUI / SwiftData / Concurrency）
* **Firebase 設計**（コレクション / セキュリティルール / Functions）
* **運用準備**（App Store Connect、年齢区分、SKAdNetwork、スクショ、ポリシー文書）

---

## 7. チャレンジ＆解決

* **時間帯連動**（出現テーブル × 背景 × BGM）を **一元管理**
* ギャラリーの **進捗に応じた背景変化（5段階）** を最小データで実現
* フレンド訪問時の **出現制御** と **FP 経済** の設計
* Firebase書き込みの効率化を狙ったGoldのオフラインキャッシュと同期処理
* 「企画〜UI/UX〜リリース準備まで広く取り組んだto-doリスト」以下参照
...
* 収集データは **ゲーム進行に必要な最小限** に限定
* **プライバシーポリシー / 利用規約** をアプリ内「設定」から参照可
* **アカウント連携（Sign in with Apple）** で端末移行に対応

---

## 10. ストア状況

* App Store: **審査中**（リリース後、リンクを本 README に追記）
...
* 制作者: **磯田 慎之助**（個人開発）
* Contact: **[shinnosuke.171717@gmail.com](mailto:shinnosuke.171717@gmail.com)**
...
* `docs/images/` に全画像を格納
* 画像の差し替え時は同名ファイルを更新（Git LFS は未使用）


## 付録: 「企画〜UI/UX〜リリース準備まで広く取り組んだto-doリスト」
...
- [x] GalleryViewにおいて左右スワイプで次のページが見られるように。
- [x] ShopViewやミッションViewにおいて左右スワイプでタブを入れ替えられるように。
- [x] ー(各種Viewにおいて、閉じやすい方法を考える。（下スワイプでバツボタン同様閉じられるように。))ー
...
- [x] 属性消して可読高める
- [x] MapViewにおいてGacha開くと、アイコンが出てしまうので、遷移方法をShop等そのほかのと合わせ、またGazhaSelectViewを背景に
...
※レア度(1~4)によって色を変えた泡と光の画像を入れている。泡はbubble_rare1~4.png,光はflash_rare1~4.png(4が最もレア)。
